	...
	namespace coap {
		...
		public:
			...
			InpOutpResponse* getData() {
				...
				IpAddress clientAddr; int clientLen = sizeof(clientAddr);
				m_ioResp.reset();
				...
				int bytesRcv = recvfrom(m_sock->getSocket(), (char*)m_tstBuf.data(), (int)m_tstBuf.length(),
					MSG_PEEK, (struct sockaddr*)&clientAddr, &clientLen);
				if (bytesRcv < 0) {
					int errNo;
#ifdef _WIN32
					...
					case IoContext::CONNECTION_STATE::LISTEN:
						{
							if (ioContext->ssl == nullptr) {
								...
								BIO* bio = BIO_new_dgram((int)m_sock->getSocket(), BIO_CLOSE);
								if (!bio) {
							...
							ret = DTLSv1_listen(ioContext->ssl, (BIO_ADDR*)&clientAddr);
							if (ret > 0) {
			...
			void init(JsonPropTree& cfg) {
				...
				#else
				fp = fopen(saCertificatePath.c_str(), "r");
				#endif
				if (!fp) {
				...
				//uint8_t on = 1;
				//m_sock->setOption(Socket::SocketOptLevel::SOCKET, Socket::SocketOptName::REUSEADDR, (const void*)&on, sizeof(on));
		...
		private:
			...
			static int verifyCert(X509* cert, X509* caCert) {
				...
				if (X509_STORE_add_cert(store, caCert) != 1) {
					LIB_MSG_ERR("Failed to add CA certificate to store");
					return 0;
				}

				X509_STORE_CTX* ctx = X509_STORE_CTX_new();
				if (!ctx) {
					LIB_MSG_ERR("Failed to create X509_STORE_CTX");
					return 0;
				}
			...
			static int dtlsVerifyCb(int ok, X509_STORE_CTX* ctx) {
				...
				if (!ssl) {
					LIB_MSG_ERR("Failed to retrieve SSL object");
			...
			static int verifyCookieCb(SSL* ssl, const unsigned char* cookie, unsigned int cookieLen) {
				...
				if (!*init) {
					cout << "Secret isn't initialized yet............\n";
					return 0;
				}
